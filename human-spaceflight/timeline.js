	...
	function parseIt(data){
		...
		for(name in data) {
			for(var m = 0; m < data[name].missions.length; m++){
				...
				l = Math.floor((launch-start)*scale*wide);  // Get pixel placement (for working out if row is filled)
				l2 = 100*(launch-start)*scale;              // Get percentage offset in timeline
				...
				a.n = n;
				a.width = dl;
				a.left = l2;	// Left offset as a percentage of timeline
